...
BX.CRM.Kanban.Grid = function(options)
{
	...
	BX.addCustomEvent("BX.CRM.Kanban.Item.select", BX.proxy(this.setMultiSelectMode, this));
	...
	BX.addCustomEvent("BX.CRM.Kanban.Item.select", BX.proxy(this.calculateTotalCheckItems, this));
	BX.addCustomEvent("BX.CRM.Kanban.Item.unSelect", BX.proxy(this.calculateTotalCheckItems, this));
	BX.addCustomEvent("BX.CRM.Kanban.Item.unSelect", BX.proxy(this.resetMultiSelectMode, this));
...
BX.CRM.Kanban.Grid.prototype = {
	...
	onBeforeItemCaptured: function(dropEvent)
	{
		...
		if (dropEvent.isActionAllowed())
		{
			...
			column.decPrice(this.itemMoving.price);
	...
	onItemMoved: function(item, targetColumn, beforeItem, skipHandler)
	{
		...
		else if (this.itemMoving.item.getData().runtimePrice === true)
		{
			this.itemMoving.oldColumn.incPrice(this.itemMoving.price);
		}
	 ...
	 * @returns {void}
	 */
	resetSelectItems: function()
	{
		var items = this.getItems();

		for (var itemId in items)
		{
			if (items[itemId].isChecked())
			{
				items[itemId].unSelectItem();
			}
		}
	},
	...
	{
		if(BX.hasClass(this.layout.gridContainer, "crm-kanban-multi-select-mode"))
			return;
