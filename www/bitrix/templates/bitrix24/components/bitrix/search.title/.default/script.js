...
BX.B24SearchTitle = function(arParams)
{
	...
	this.BuildCrmEntities = function (result)
	{
		var crmContact = [];
		var crmCompany= [];
		var crmDeal= [];
		...
		var crmContactMore = false, crmCompanyMore = false, crmDealMore = false, crmLeadMore = false,
			crmInvoiceMore = false, crmQuoteMore = false;
		...
		if (result.data.length > 0)
		{
			for (var i in result.data)
			{
				...
				var crmItem = {
					...
					"URL": result.data[i].showUrl,
					"ITEM_ID" : result.data[i].entityType + result.data[i].entityId
				};

				if (result.data[i].entityType == "CONTACT")
				{
					if (crmContact.length < 10)
					{
						crmContact.push(crmItem);
					}
					else
					{
						crmContactMore = true;
				...
				else if (result.data[i].entityType == "COMPANY")
				{
					if (crmCompany.length < 10)
					{
						crmCompany.push(crmItem);
					}
					else
					{
						crmCompanyMore = true;
				...
				else if (result.data[i].entityType == "DEAL")
				{
					if (crmDeal.length < 10)
					{
						crmDeal.push(crmItem);
					}
					else
					{
						crmDealMore = true;
				...
				else if (result.data[i].entityType == "LEAD")
				{
					if (crmLead.length < 10)
					{
						crmLead.push(crmItem);
					}
					else
					{
						crmLeadMore = true;
				...
				else if (result.data[i].entityType == "QUOTE")
				{
					if (crmQuote.length < 10)
					{
						crmQuote.push(crmItem);
					}
					else
					{
						crmQuoteMore = true;
				...
				else if (result.data[i].entityType == "INVOICE")
				{
					if (crmInvoice.length < 10)
					{
						crmInvoice.push(crmItem);
					}
					else
					{
						crmInvoiceMore = true;
			...
			this.BuildCrmBlock(crmDeal, "CRM: " + BX.message("SEARCH_CRM_DEAL"), "deal");
			if (crmDealMore)
			{
				var item = {
			...
			this.BuildCrmBlock(crmContact, "CRM: " + BX.message("SEARCH_CRM_CONTACT"), "contact");
			if (crmContactMore)
			{
				var item = {
			...
			this.BuildCrmBlock(crmCompany, "CRM: " + BX.message("SEARCH_CRM_COMPANY"), "company");
			if (crmCompanyMore)
			{
				var item = {
			...
			this.BuildCrmBlock(crmLead, "CRM: " + BX.message("SEARCH_CRM_LEAD"), "lead");
			if (crmLeadMore)
			{
				var item = {
			...
			this.BuildCrmBlock(crmInvoice, "CRM: " + BX.message("SEARCH_CRM_INVOICE"), "invoice");
			if (crmInvoiceMore)
			{
				var item = {
			...
			this.BuildCrmBlock(crmQuote, "CRM: " + BX.message("SEARCH_CRM_QUOTE"), "quote");
			if (crmQuoteMore)
			{
				var item = {
	...
	this.BuildCrmBlock = function (items, blockTitle, entityType)
	{
		if (items.length > 0)
		{
			var crmBlocks = [];
			...
			if (crmBlocks)
			{
				this.BuildCrmEntity(crmBlocks, blockTitle, entityType);
			}
		}
	};

	this.BuildCrmEntity = function (crmBlocks, blockTitle, entityType)
	{
	...
	this.ShowResult = function(result, showWaiter, afterAjax)
	{
		...
		if (typeof _this.arParams.FORMAT != 'undefined' && _this.arParams.FORMAT == 'json')
		{
			...
			if (BX.type.isDomNode(result) && result.innerHTML.length)
			{
				...
				//search in crm
				if (afterAjax && _this.arParams.IS_CRM_INSTALLED)
				{
					BX("title-search-waiter").style.display = "block";
					var resCrm = BX.ajax.runAction("crm.api.entity.search", { data: { search: this.INPUT.value, options: { scope: 'index'/*, types: [ BX.CrmEntityType.names.contact ] */} } });
					resCrm.then(this.BuildCrmEntities.bind(this));
				}
			}
		}
		else
