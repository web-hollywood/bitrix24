...
if(isset($arFilter['FIND']))
{
	if($arFilter['FIND'] !== '')
	{
		$foundIds = CCrmInvoice::Search($arFilter['FIND'], 20, 2);
		if (!empty($foundIds))
		{
			if (isset($arFilter['ID']))
			{
				if (is_array($arFilter['ID']))
				{
					if (count($foundIds) > 1)
					{
						$arFilter['ID'] = array_intersect($arFilter['ID'], $foundIds);
					}
					else
					{
						if (array_search($foundIds[0], $arFilter['ID']) !== false)
						{
							$arFilter['ID'] = $foundIds[0];
						}
						else
						{
							$arFilter['ID'] = 0;
				...
				{
					if (array_search($arFilter['ID'], $foundIds) === false)
					{
						$arFilter['ID'] = 0;
					}
				}
...
if ($arParams['IS_RECURRING'] !== 'Y')
{
	$obRes = CCrmInvoice::GetList(
		...
		$arFilter,
		false,
		($arResult['GADGET'] != 'Y' && $sExportType == '') ? $arNavParams : false,
		$arSelect,
		$arOptions
	);
}
