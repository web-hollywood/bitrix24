...
BX.Crm.MeasureClass = (function ()
{
	...
	MeasureClass.prototype.resetEntityListGrid = function (e)
	{
		...
		filter.FIND = '';
		var req = {"fields": filter};
		req.SITE_ID = BX.message('SITE_ID');
		req.sessid = BX.bitrix_sessid();
		req.apply_filter = 'Y';
		req.clear_filter = 'Y';
		req.save = 'Y';
		req.preset_id = 'tmp_filter';
		req.rows = fields;

		BX.ajax({
			method: 'POST',
			dataType: 'json',
			url: '/bitrix/components/bitrix/main.ui.filter/settings.ajax.php?FILTER_ID=' + filterId + '&GRID_ID=' + gridId+ '&action=SET_FILTER',
			data: req,
