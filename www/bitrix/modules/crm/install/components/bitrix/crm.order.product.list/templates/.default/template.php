			...
			$skuItemHtml = '
				<div class="crm-order-product-properties-scu-title">
					<div class="crm-order-product-properties-scu-title-text">'.
						$arResult['IBLOCKS_SKU_PARAMS'][$product['OFFERS_IBLOCK_ID']][$pSkuId]['NAME'].
					'</div>
				</div>
...
if (!$isReadOnly)
{
	$buttons = [
		[
			...
			'ICON' => 'btn-new',
			'ONCLICK' => $jsObjName.".addProductSearch({lang: '".LANGUAGE_ID."', siteId: '".$arResult['ORDER_SITE_ID']."', orderId: '".$arResult['ORDER_ID']."'});"
	]];
	...
	if($arResult['ALLOW_CREATE_NEW_PRODUCT'])
	{
		$buttons[] = [
			...
			'ICON' => 'btn-new',
			'ONCLICK' => "BX.Crm.Page.openSlider(\"".$productCreateLink."\", { width: 500 }); return;"
		];
	}
...
<script type="text/javascript">
		BX.ready(function ()
		{
			...
			if(form = document.forms['<?=$arResult['FORM_ID']?>'])
			{
