	...
	if ($iVersion >= 2)
	{
		...
		if (!empty($arLead['STATUS_ID']))
		{
			$fields .= '<span class="field-row">
				<span class="field-name">'.GetMessage('CRM_COLUMN_STATUS').'</span>: <span class="field-value"><span class="fields enumeration">'.$arResult['STATUS_LIST'][$arLead['STATUS_ID']].'</span></span>
			</span>';
		}
		...
		$fields .= '<span class="field-row">
			<span class="field-name">'.GetMessage('CRM_COLUMN_DATE_MODIFY').'</span>: <span class="field-value"><span class="fields enumeration">'.FormatDate('x', MakeTimeStamp($arLead['DATE_MODIFY']), (time() + CTimeZone::GetOffset())).'</span></span>
		</span>';

//		$fields .= '<span class="field-row">'.GetMessage('CRM_SECTION_CONTACT_INFO').'</span>';

		if (!empty($arLead['FORMATTED_NAME']))
		{
			$fields .= '<span class="field-row">
				<span class="field-name">'.GetMessage('CRM_COLUMN_NAME').'</span>: <span class="field-value">'.htmlspecialcharsbx($arLead['FORMATTED_NAME']).'</span>
			</span>';
		}
		if (isset($multiFieldHtml['PHONE']))
		{
			$fields .= '<span class="field-row">
				<span class="field-name">'.GetMessage('CRM_COLUMN_PHONE').'</span>: <span class="field-value">'.$multiFieldHtml['PHONE'].'</span>
			</span>';
		}
		if (isset($multiFieldHtml['EMAIL']))
		{
			$fields .= '<span class="field-row">
				<span class="field-name">'.GetMessage('CRM_COLUMN_EMAIL').'</span>: <span class="field-value">'.$multiFieldHtml['EMAIL'].'</span>
			</span>';
		}
