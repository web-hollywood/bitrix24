...
BX.Disk.FolderListClass = (function (){
	...
	FolderListClass.prototype.setEvents = function()
	{
		...
		BX.SidePanel.Instance.bindAnchors({
			rules: [
				...
				},
				{
					...
					handler: function(event, link) {
						if (!link.anchor.dataset.bxViewer)
						{
		...
		//BX.addCustomEvent("onPullEvent-disk", this.onPullDiskEvent.bind(this));
	...
	FolderListClass.prototype.onPullDiskEvent = function(command, params)
	{
		params = params || {};
	...
	FolderListClass.prototype.createFile = function () {
		BX.Disk.modalWindow({
			...
			content: [
				BX.create('div', {
					...
					children: [
						BX.message('DISK_FOLDER_MW_CREATE_FILE_TEXT') + ' ',
						BX.create('span', {
							style: {
								cursor: 'pointer'
							},
				...
				BX.create('div', {
					...
					children: [
						BX.create('a', {
							text: BX.message('DISK_FOLDER_MW_CREATE_TYPE_DOC'),
							props: {
						...
						BX.create('a', {
							text: BX.message('DISK_FOLDER_MW_CREATE_TYPE_XLS'),
							props: {
						...
						BX.create('a', {
							text: BX.message('DISK_FOLDER_MW_CREATE_TYPE_PPT'),
							props: {
	...
	FolderListClass.prototype.runCreatingFile = function (documentType, service) {
		...
		if (BX.CViewer.isLocalEditService(service))
		{
			if (BX.CViewer.isEnableLocalEditInDesktop())
			{
				obElementViewer.setEditService('l');
			}
			else
			{
				obElementViewer.helpDiskDialog();
			}
		}
		else
		{
			obElementViewer.setEditService(service);
		}
