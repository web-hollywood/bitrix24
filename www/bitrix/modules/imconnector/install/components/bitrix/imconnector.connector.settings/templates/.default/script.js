;(function(window){
	...
	destInput.prototype = {
		...
		saveUsers: function (formData) {
			BX.ajax.runComponentAction('bitrix:imconnector.connector.settings', 'saveUsers', {
				mode: 'ajax',
				data: formData
	...
	window.BX.OpenLinesConfigEdit = {
		...
		addEventForSelect : function(selectLink)
		{
			var isIframe = selectLink.dataset.iframe === 'Y';

			BX.bind(selectLink, 'change', function () {
				var selected = this.options[this.selectedIndex];

				if (selected.dataset.new === 'Y')
				{
					BX.OpenLinesConfigEdit.createLineAction(selected.value, isIframe);
				}
				else if (isIframe)
				{
					BX.SidePanel.Instance.close();
					BX.SidePanel.Instance.open(selected.value, {width: 700});
				}
				else
				{
	...
	BX.ready(function(){
		...
		var selectLink = document.querySelector('[data-role="select"]');

		BX.UI.Hint.init(BX('bx-connector-user-list'));

		if (selectLink && typeof selectLink != "undefined")
		{
			BX.OpenLinesConfigEdit.addEventForSelect(selectLink);
		}
	});
