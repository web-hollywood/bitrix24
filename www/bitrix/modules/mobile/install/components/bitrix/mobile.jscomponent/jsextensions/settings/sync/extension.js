...
BX.addCustomEvent("onRegisterProvider", (addProviderHandler) =>
{
	...
	forms["sync"] = new Form("sync", BX.message("SE_SYNC_TITLE")).addSection(
		new FormSection("main", BX.message("SE_SYNC_SUBTITLE_TITLE"), BX.message("SE_SYNC_PROFILE_DESCRIPTION")).addItems([
			new FormItem("sync_calendar", FormItemType.BUTTON, BX.message("SE_SYNC_CAL_TITLE")).setCustomParam("resource", "caldav"),
			new FormItem("sync_contacts", FormItemType.BUTTON, BX.message("SE_SYNC_CONTACTS_TITLE")).setCustomParam("resource", "carddav")
		])
	).compile();
