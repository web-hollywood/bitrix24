...
(()=>{
	class Profile
	{
		...
		error(message)
		{
			let errorMessage = message || BX.message("SOMETHING_WENT_WRONG");
	...
	class ProfileView extends Profile
	{
		...
		render()
		{
			...
			let action = {
				title: BX.message("COMMUNICATE_WITH"),
				imageUrl:"/bitrix/components/bitrix/mobile.jscomponent/jsextensions/user/profile/images/communication.png?1",
				sectionCode: "actions",
				styles:{
					title:{
						font:{
			...
			BX.onViewLoaded(() =>
			{
				...
				if(!this.loaded)
				{
					...
					this.loaded = true;
					this.form.setTitle({text: BX.message("PROFILE_INFO")});
				}
				else
				{
					items.push(topItem);
					BX.onViewLoaded(() => this.form.setItems(items, this.formSections));
				}
			});
		}
		...
		static open (userData = {}, formObject = false)
		{
			...
			if(Application.getApiVersion() >= 27 && params.userId != null)
			{
				...
				{
					formObject.setTitle({text:params.name});
