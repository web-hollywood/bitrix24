...
class MobileWebComponent extends \CBitrixComponent
{
	...
	public function generateArchive()
	{
		...
		$map = include(Application::getDocumentRoot() . $this->componentPath . "/resmap.php");
	...
	private function injectResourcesIntoIndexFile($path, $map)
	{
		...
		foreach ($injectionRules as $type => $rule)
		{
			...
			{
				$injectionContent[$type] .= str_replace("#PATH#", $localPath.'?'.$injectionContent["timestamp"], $rule);
			}
		}
	...
	private function copyResourceMapFiles($path, $map)
	{
		...
		if (!isset($map["langs"]) || !is_array($map["langs"]))
		{
			$map["langs"] = [];
		}
		...
		foreach ($map["langs"] as $langFile)
		{
			if (file_exists(str_replace("#LANG_ID#", $defaultLang, Application::getDocumentRoot().$langFile)))
			{
				include(str_replace("#LANG_ID#", $defaultLang, Application::getDocumentRoot().$langFile));
			}
			if ($defaultLang != LANGUAGE_ID && file_exists(str_replace("#LANG_ID#", LANGUAGE_ID, Application::getDocumentRoot().$langFile)))
			{
				include(str_replace("#LANG_ID#", LANGUAGE_ID, Application::getDocumentRoot() . $langFile));
			}
		}
...
JS;
		...
		{
			$errorFiles = implode("\n", array_map(function ($langFile) {
				return str_replace("#LANG_ID#", LANGUAGE_ID, Application::getDocumentRoot() . $langFile);
			}, $map["langs"]));
			$this->addError("The localization files are defined, but there are not contain phrases or paths are wrong: \n" . $errorFiles);
		}
