...
class CBPCrmSendEmailActivity extends CBPActivity
{
	...
	private function getToEmail($entityTypeId, $entityId)
	{
		...
		if ($entityTypeId == \CCrmOwnerType::Deal)
		{
			...
			if($contactId > 0)
			{
				$to = $this->getEntityEmail(\CCrmOwnerType::Contact, $contactId);
				$comEntityTypeId = \CCrmOwnerType::Contact;
				$comEntityId = $contactId;
			}
			if (empty($to) && $companyId > 0)
			{
				$to = $this->getEntityEmail(\CCrmOwnerType::Company, $companyId);
		...
		elseif ($entityTypeId == \CCrmOwnerType::Order)
		{
			...
			{
				$to = $this->getEntityEmail($row['ENTITY_TYPE_ID'], $row['ENTITY_ID']);
				if ($to)
				{
