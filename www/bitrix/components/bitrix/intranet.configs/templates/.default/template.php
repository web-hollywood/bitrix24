			...
			<td class="content-edit-form-field-input">
				...
				<div id="RIGHTS_div">
					...
					foreach($arToAllRights as $right)
					{
						...
						?><div data-bx-right="<?=htmlspecialcharsbx($right) ?>" class="toall-right"><span><?=htmlspecialcharsbx((!empty($arNames[$right]["provider"]) ? $arNames[$right]["provider"].": " : "").$arNames[$right]["name"])?>&nbsp;</span><a href="javascript:void(0);" onclick="B24ConfigsLiveFeedObj.DeleteToAllAccessRow(this);" class="access-delete" title="<?=GetMessage("CONFIG_TOALL_DEL")?>"></a></div><?
					}
					?>
				</div>
			...
			<td class="content-edit-form-field-input">
				...
				<div id="chat_RIGHTS_div">
					...
					foreach($arChatToAllRights as $right)
					{
						...
						?><div data-bx-right="<?=htmlspecialcharsbx($right) ?>" class="toall-right"><span><?=htmlspecialcharsbx((!empty($arNames[$right]["provider"]) ? $arNames[$right]["provider"].": " : "").$arNames[$right]["name"])?>&nbsp;</span><a href="javascript:void(0);" onclick="B24ImChatObj.DeleteToAllAccessRow(this);" class="access-delete" title="<?=GetMessage("CONFIG_TOALL_DEL")?>"></a></div><?
					}
					?>
				</div>
...
?>
		<tr>
			<td class="content-edit-form-field-name content-edit-form-field-name-left"><label for="mp_allow_user_install"><?=GetMessage('CONFIG_MP_ALLOW_USER_INSTALL')?></label></td>
			<td class="content-edit-form-field-input">
...
?>
		<tr id="mp_user_install" <?if(!$mpUserAllowInstall) echo ' style="display: none;"'?> >
			...
			<td class="content-edit-form-field-input"><?
			$access = new CAccess();
			$arNames = $access->GetNames($arResult['MP_ALLOW_USER_INSTALL']);
			?>
			<div id="mp_RIGHTS_div">
				<?
				foreach($arResult['MP_ALLOW_USER_INSTALL'] as $right)
				{
					?><input type="hidden" name="mp_user_install_rights[]" id="mp_user_install_rights_<?=htmlspecialcharsbx($right)?>" value="<?=htmlspecialcharsbx($right)?>"><?
					?><div data-bx-right="<?=htmlspecialcharsbx($right) ?>" class="user-install-right"><span><?=htmlspecialcharsbx((!empty($arNames[$right]["provider"]) ? $arNames[$right]["provider"].": " : "").$arNames[$right]["name"])?>&nbsp;</span><a href="javascript:void(0);" onclick="B24MpObj.DeleteToAllAccessRow(this);" class="access-delete" title="<?=GetMessage("CONFIG_TOALL_DEL")?>"></a></div><?
				}
				?>
			</div>
			<div style="padding-top: 5px;"><a href="javascript:void(0)" class="bx-action-href" onclick="B24MpObj.ShowToAllAccessPopup(B24MpObj.arUserInstallRights);"><?=GetMessage("CONFIG_TOALL_RIGHTS_ADD")?></a></div>
			</td>
			<td class="content-edit-form-field-error"></td>
		</tr>
<?
	...
	<!-- GDPR for Europe-->
		<?
		if ($arResult["IS_BITRIX24"] && !in_array($arResult["LICENSE_PREFIX"], array("ru", "ua", "kz", "by")))
		{
		?>
			<tr>
			...
			<tr>
				<td class="content-edit-form-field-name content-edit-form-field-name-left"><?=GetMessage('CONFIG_GDRP_LABEL1')?></td>
...
{
?>
	var B24MpObj = new BX.Bitrix24.Configs.MpClass(<?=CUtil::PhpToJSObject($arResult['MP_ALLOW_USER_INSTALL'])?>);
<?
}
?>
</script>