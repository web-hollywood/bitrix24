	...
	if ($iVersion >= 2)
	{
		...
		if (!empty($arQuote['STATUS_ID']))
		{
			$fields .= '<span class="field-row">
				<span class="field-name">'.GetMessage('CRM_COLUMN_STATUS_ID').'</span>: <span class="field-value"><span class="fields enumeration">'.$arResult['STATUS_LIST'][$arQuote['STATUS_ID']].'</span></span>
			</span>';
		}
		...
		{
			$fields .= '<span class="field-row">
				<span class="field-name">'.GetMessage('CRM_COLUMN_OPPORTUNITY').'</span>: <span class="field-value"><span class="fields enumeration"><nobr>'.number_format($arQuote['OPPORTUNITY'], 2, ',', ' ').' '.htmlspecialcharsbx(CCrmCurrency::GetCurrencyName($arQuote['CURRENCY_ID'])).'</nobr></span></span>
			</span>';
		}
		$fields .= '<span class="field-row">
			<span class="field-name">'.GetMessage('CRM_COLUMN_DATE_MODIFY').'</span>: <span class="field-value"><span class="fields enumeration">'.FormatDate('x', MakeTimeStamp($arQuote['DATE_MODIFY']), (time() + CTimeZone::GetOffset())).'</span></span>
		</span>';
		if (!empty($arQuote['COMPANY_TITLE']))
		{
			$fields .= '<span class="field-row">
				<span class="field-name">'.GetMessage('CRM_COLUMN_COMPANY_TITLE').'</span>: <span class="field-value"><a href="'.$arQuote['PATH_TO_COMPANY_SHOW'].'" target="_blank">'.htmlspecialcharsbx($arQuote['COMPANY_TITLE']).'</a></span>
			</span>';
		}
		if (!empty($arQuote['CONTACT_FORMATTED_NAME']))
		{
			$fields .= '<span class="field-row">
				<span class="field-name">'.GetMessage('CRM_COLUMN_CONTACT_FULL_NAME').'</span>: <span class="field-value"><a href="'.$arQuote['PATH_TO_CONTACT_SHOW'].'" target="_blank">'.htmlspecialcharsbx($arQuote['CONTACT_FORMATTED_NAME']).'</a></span>
			</span>';
		}
