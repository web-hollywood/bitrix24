...
foreach($arResult['DEAL'] as $sKey => $arDeal)
{
	...
	$arActions[] = array(
		'TITLE' => Loc::getMessage('CRM_DEAL_SHOW_TITLE'),
		'TEXT' => Loc::getMessage('CRM_DEAL_SHOW'),
	...
	{
		$arActions[] = array(
			'TITLE' => Loc::getMessage('CRM_DEAL_EDIT_TITLE'),
			'TEXT' => Loc::getMessage('CRM_DEAL_EDIT'),
		...
		{
			$arActions[] = array(
				'TITLE' => Loc::getMessage('CRM_DEAL_COPY_TITLE'),
				'TEXT' => Loc::getMessage('CRM_DEAL_COPY'),
	...
	if(!$isInternal && $arDeal['DELETE'])
	{
		...
		$arActions[] = array(
			'TITLE' => Loc::getMessage('CRM_DEAL_DELETE_TITLE'),
			'TEXT' => Loc::getMessage('CRM_DEAL_DELETE'),
	...
	if($allowExclude && $arDeal['CAN_EXCLUDE'])
	{
		...
		$arActions[] = array(
			'TITLE' => Loc::getMessage('CRM_DEAL_EXCLUDE_TITLE'),
			'TEXT' => Loc::getMessage('CRM_DEAL_EXCLUDE'),
			'ONCLICK' => "BX.CrmUIGridExtension.processMenuCommand(
					'{$gridManagerID}',
	...
	if(!$isInternal && $arParams['IS_RECURRING'] !== 'Y')
	{
		if($arResult['CAN_CONVERT'])
		{
			if($arResult['CONVERSION_PERMITTED'])
			{
				...
				if(!empty($arSchemeList))
				{
					$arActions[] = array('SEPARATOR' => true);
					$arActions[] = array(
						'TITLE' => Loc::getMessage('CRM_DEAL_CREATE_ON_BASIS_TITLE'),
						'TEXT' => Loc::getMessage('CRM_DEAL_CREATE_ON_BASIS'),
						'MENU' => $arSchemeList
					);
				}
			}
		...
		if($arDeal['EDIT'])
		{
			$arActions[] = $arActivityMenuItems[] = array(
				'TITLE' => Loc::getMessage('CRM_DEAL_EVENT_TITLE'),
				'TEXT' => Loc::getMessage('CRM_DEAL_EVENT'),
				'ONCLICK' => "BX.CrmUIGridExtension.processMenuCommand(
					'{$gridManagerID}',
			...
			{
				$arActivityMenuItems[] = array(
					'TITLE' => Loc::getMessage('CRM_DEAL_ADD_CALL_TITLE'),
					'TEXT' => Loc::getMessage('CRM_DEAL_ADD_CALL'),
					'ONCLICK' => "BX.CrmUIGridExtension.processMenuCommand(
						'{$gridManagerID}',
				...
				$arActivityMenuItems[] = array(
					'TITLE' => Loc::getMessage('CRM_DEAL_ADD_MEETING_TITLE'),
					'TEXT' => Loc::getMessage('CRM_DEAL_ADD_MEETING'),
					'ONCLICK' => "BX.CrmUIGridExtension.processMenuCommand(
						'{$gridManagerID}',
						BX.CrmUIGridMenuCommand.createActivity,
				...
				$arActivitySubMenuItems[] = array(
					'TITLE' => Loc::getMessage('CRM_DEAL_ADD_CALL_TITLE'),
					'TEXT' => Loc::getMessage('CRM_DEAL_ADD_CALL_SHORT'),
					'ONCLICK' => "BX.CrmUIGridExtension.processMenuCommand(
						'{$gridManagerID}',
						BX.CrmUIGridMenuCommand.createActivity,
						{ typeId: BX.CrmActivityType.call, settings: { ownerID: {$arDeal['ID']} } }
				...
				$arActivitySubMenuItems[] = array(
					'TITLE' => Loc::getMessage('CRM_DEAL_ADD_MEETING_TITLE'),
					'TEXT' => Loc::getMessage('CRM_DEAL_ADD_MEETING_SHORT'),
					'ONCLICK' => "BX.CrmUIGridExtension.processMenuCommand(
						'{$gridManagerID}',
						BX.CrmUIGridMenuCommand.createActivity,
						{ typeId: BX.CrmActivityType.meeting, settings: { ownerID: {$arDeal['ID']} } }
			...
			{
				$arActivityMenuItems[] = array(
					'TITLE' => Loc::getMessage('CRM_DEAL_TASK_TITLE'),
					'TEXT' => Loc::getMessage('CRM_DEAL_TASK'),
					'ONCLICK' => "BX.CrmUIGridExtension.processMenuCommand(
						'{$gridManagerID}',
						BX.CrmUIGridMenuCommand.createActivity,
				...
				$arActivitySubMenuItems[] = array(
					'TITLE' => Loc::getMessage('CRM_DEAL_TASK_TITLE'),
					'TEXT' => Loc::getMessage('CRM_DEAL_TASK_SHORT'),
					'ONCLICK' => "BX.CrmUIGridExtension.processMenuCommand(
						'{$gridManagerID}',
						BX.CrmUIGridMenuCommand.createActivity,
						{ typeId: BX.CrmActivityType.task, settings: { ownerID: {$arDeal['ID']} } }
			...
			{
				$arActions[] = array(
					'TITLE' => Loc::getMessage('CRM_DEAL_ADD_ACTIVITY_TITLE'),
					'TEXT' => Loc::getMessage('CRM_DEAL_ADD_ACTIVITY'),
			...
			if($arResult['IS_BIZPROC_AVAILABLE'])
			{
				...
				if(isset($arContact['PATH_TO_BIZPROC_LIST']) && $arContact['PATH_TO_BIZPROC_LIST'] !== '')
					$arActions[] = array(
						'TITLE' => Loc::getMessage('CRM_DEAL_BIZPROC_TITLE'),
						'TEXT' => Loc::getMessage('CRM_DEAL_BIZPROC'),
	...
	&& ($allowWrite || $allowExclude || $allowDelete))
{
	...
	$actionList = array(array('NAME' => Loc::getMessage('CRM_DEAL_LIST_CHOOSE_ACTION'), 'VALUE' => 'none'));
	...
	if($allowWrite && $arParams['IS_RECURRING'] !== "Y")
	{
		...
		{
			$actionList[] = array(
				'NAME' => Loc::getMessage('CRM_DEAL_TASK'),
				'VALUE' => 'tasks',
				'ONCHANGE' => array(
					array(
		...
		if ($arParams['IS_RECURRING'] !== "Y")
		{
			...
			if($arResult['EFFECTIVE_CATEGORY_ID'] >= 0)
			{
				...
				$stageList = array(array('NAME' => Loc::getMessage('CRM_STAGE_INIT'), 'VALUE' => ''));
				...
				$actionList[] = array(
					'NAME' => Loc::getMessage('CRM_DEAL_SET_STAGE'),
					'VALUE' => 'set_stage',
					'ONCHANGE' => array(
						array(
							'ACTION' => Bitrix\Main\Grid\Panel\Actions::CREATE,
		...
		//endregion
		$actionList[] = array(
			'NAME' => Loc::getMessage('CRM_DEAL_ASSIGN_TO'),
			'VALUE' => 'assign_to',
			'ONCHANGE' => array(
				array(
					'ACTION' => Bitrix\Main\Grid\Panel\Actions::CREATE,
		...
		{
			$actionList[] = array(
				'NAME' => Loc::getMessage('CRM_DEAL_CREATE_CALL_LIST'),
		...
		$actionList[] = array(
			'NAME' => Loc::getMessage('CRM_DEAL_REFRESH_ACCOUNT'),
	...
	{
		$actionList[] = array(
			'NAME' => Loc::getMessage('CRM_DEAL_EXCLUDE'),
			'VALUE' => 'exclude',
			'ONCHANGE' => array(
				array(
					'ACTION' => Bitrix\Main\Grid\Panel\Actions::CREATE,
	...
	if($allowWrite)
	{
		...
		$actionList[] = array(
			'NAME' => Loc::getMessage('CRM_DEAL_MARK_AS_OPENED'),
		...
		if($arResult['HAVE_CUSTOM_CATEGORIES'] && $arResult['CATEGORY_ID'] >= 0)
		{
			...
			$actionList[] = array(
				'NAME' => Loc::getMessage('CRM_DEAL_MOVE_TO_CATEGORY'),
	...
	if($callListUpdateMode)
	{
		$controlPanel['GROUPS'][0]['ITEMS'][] = array(
			...
			"TEXT" => Loc::getMessage("CRM_DEAL_UPDATE_CALL_LIST"),
			"ID" => "update_call_list",
	...
	else
	{
		...
		if(IsModuleInstalled('voximplant'))
		{
			$controlPanel['GROUPS'][0]['ITEMS'][] = array(
				"TYPE" => \Bitrix\Main\Grid\Panel\Types::BUTTON,
				"TEXT" => Loc::getMessage('CRM_DEAL_START_CALL_LIST'),
	...
	$APPLICATION->IncludeComponent(
		...
		array(
			...
			'NAVIGATION_BAR' => array(
				'ITEMS' => array_merge(
					\Bitrix\Crm\Automation\Helper::getNavigationBarItems(\CCrmOwnerType::Deal),
					array(
						array(
							'id' => 'kanban',
