...
class CDiskFileHistoryComponent extends DiskComponent implements SidePanelWrappable
{
	...
	private function getVersionGridData($gridId)
	{
		...
		foreach ($this->arResult['ITEMS'] as $version)
		{
			...
			$dataAttributesForViewer = Ui\Viewer::getAttributesByVersion(
				$version,
				[
					'canUpdate' => $this->file->canUpdate(
						$securityContext
					)
				]
			);
			...
			$rows[] = [
				...
				'columns' => [
					...
					'NAME' => "<a href='{$urlManager->getUrlForDownloadVersion($version)}' {$dataAttributesForViewer}>" . $version->getName() . "</a>",
