...
class CrmKanbanComponent extends \CBitrixComponent
{
	...
	protected $selectPresets = array(
								...
								'deal' => array('ID', 'STAGE_ID', 'TITLE', 'DATE_CREATE', 'BEGINDATE', 'OPPORTUNITY', 'OPPORTUNITY_ACCOUNT', 'CURRENCY_ID', 'ACCOUNT_CURRENCY_ID', 'CONTACT_ID', 'COMPANY_ID', 'MODIFY_BY_ID', 'ASSIGNED_BY'),
	...
	protected function getStatuses($clear = false)
	{
		...
		)
		{
			...
			{
				$colors['DEAL_STAGE_' . $cat['ID']] = (array)unserialize(Option::get(
					'crm',
		...
		{
			$colors['DEAL_STAGE'] = (array)unserialize(Option::get(
				'crm',
		...
		if ($filter === null)
		{
			...
			)
			{
				...
				while ($row = $res->fetch())
				{
					...
					{
						$this->allowSemantics[] = '@' . $row['STATUS_ID'];
					}
				}
			}
			...
			foreach ($this->getStatuses($clear) as $status)
			{
				...
				if (
					...
					(
						...
						||
						in_array('@' . $status['STATUS_ID'], $this->allowSemantics)
					)
				)
				{
					$dropzone = false;
	...
	protected function getItems(array $filter = array(), $pagen = true)
	{
		...
		if (
			isset($filterCommon[$statusKey]) &&
			isset($filter[$statusKey])
		)
		{
			$statusKeyColumn = $filter[$statusKey];
			unset($filter[$statusKey]);
		}
		...
		if (class_exists($provider))
		{
			//user sorting
			if (
				isset($filter[$statusKeyColumn]) && isset($columns[$filter[$statusKeyColumn]]) &&
				$columns[$filter[$statusKeyColumn]]['count']<=$this->maxSortSize
			)
			{
				//get all and sort
				$sorting = true;
	...
	public function executeComponent()
	{
		...
		if ($this->arParams['EMPTY_RESULT'] != 'Y')
		{
			...
			{
				$this->arResult['CATEGORIES'] = DealCategory::getAll(true);
			}
		}
