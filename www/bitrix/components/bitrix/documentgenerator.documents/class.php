...
class DocumentGeneratorDocumentsComponent extends CBitrixComponent
{
		...
		if(!empty($documents))
		{
			foreach($documents as $document)
			{
				...
				if($viewUrl)
				{
					...
					$onDocumentClick = 'BX.DocumentGenerator.Document.onBeforeCreate(\''.$viewUrl->getLocator().'\', {}, \''.$this->arParams['loaderPath'].'\')';
					$actions[] = [
						'ICONCLASS' => 'edit',
	...
	protected function getTemplates()
	{
		...
		{
			$templates = [];
			$list = \Bitrix\DocumentGenerator\Model\TemplateTable::getListByClassName(get_class($this->provider), \Bitrix\DocumentGenerator\Driver::getInstance()->getUserId(), $this->provider->getSource());
