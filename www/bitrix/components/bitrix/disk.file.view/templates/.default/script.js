...
BX.Disk.FileViewClass = (function ()
{
		...
		BX.ajax.runAction('disk.api.file.showSharingEntities', {
			...
			if (BX.type.isArray(response.data))
			{
				response.data.forEach(function (item) {
					...
					BX.SocNetLogDestination.obItemsSelected[this.selectorId][item.entity.id] = item.entity.type;
	...
	{
		BX.fireEvent(BX('bx-disk-filepage-filename'), 'click');
		//todo shit! or sheet?
	...
	{
		BX.Main.selectorManager.getById(this.selectorId).show();
	};
	...
	FileViewClass.prototype.onSelectSelectorItem = function (data)
	{
		...
		BX.Main.selectorManager.getById(this.selectorId).closeDialog();
