		...
		<div class="disk-detail-content">
			...
			<? } else { ?>
				<div class="disk-file-info">
					<div class="disk-file-info-title"><?= Loc::getMessage('DISK_FILE_VIEW_USAGE') ?></div>
					<div class="disk-file-info-item">
						<span class="disk-file-info-item-not-used"><?= Loc::getMessage('DISK_FILE_VIEW_NOT_USED') ?></span>
					</div>
				</div>
			<? } ?>
		</div>
		<div class="disk-detail-sidebar">
			...
			<div class="disk-detail-sidebar-section">
				<div class="disk-detail-sidebar-editor">
					...
					<? if (!$arResult['FILE']['IS_DELETED'] && !empty($arResult['CAN_UPDATE']) && (!$arResult['FILE']['LOCK']['IS_LOCKED'] || $arResult['FILE']['LOCK']['IS_LOCKED_BY_SELF']) && \Bitrix\Disk\Document\DocumentHandler::isEditable(GetFileExtension($arResult['FILE']['NAME'])))
					{
						?><a id="bx-disk-file-edit-btn" class="disk-detail-sidebar-editor-item disk-detail-sidebar-editor-item-edit" href=""><?= Loc::getMessage('DISK_FILE_VIEW_FILE_EDIT') ?></a><?
					}?>
...
$APPLICATION->IncludeComponent(
	...
	<script type="text/javascript">
		var d
		BX(function () {
			d = BX.Disk['FileViewClass_<?= $component->getComponentId() ?>'] = new BX.Disk.FileViewClass({
			...
			top.BX.viewElementBind(
				BX('bx-disk-filepage-filename').parentNode,
				{showTitle: true},
				{attr: 'data-bx-viewer'}
			);
			top.BX.viewElementBind(
