...
class CCrmOrderDetailsComponent extends Crm\Component\EntityDetails\BaseComponent
{
	...
	public function obtainOrder()
	{
		if($this->order === null)
		{
			...
			{
				$order = \Bitrix\Crm\Order\Manager::createEmptyOrder(
					!empty($_REQUEST['SITE_ID']) ? $_REQUEST['SITE_ID'] : SITE_ID
				);
			}
	...
	protected function prepareFieldInfos()
	{
		...
		$this->arResult['ENTITY_FIELDS'] = array(
			...
			array(
				'name' => 'MYCOMPANY_ID',
				'title' => Loc::getMessage('CRM_ORDER_FIELD_MYCOMPANY'),
				'type' => 'crm_entity',
				'editable' => true,
				'data' => array(
	...
	public function prepareEntityData()
	{
		...
		if($this->mode === ComponentMode::CREATION)
		{
			...
			$this->entityData['USER_PROFILE'] = 'NEW';
		...
		{
			$user = self::getUser($this->entityData['RESPONSIBLE_ID']);
			if(is_array($user))
			{
				$this->entityData['RESPONSIBLE_LOGIN'] = $user['LOGIN'];
				$this->entityData['RESPONSIBLE_NAME'] = isset($user['NAME']) ? $user['NAME'] : '';
				$this->entityData['RESPONSIBLE_SECOND_NAME'] = isset($user['SECOND_NAME']) ? $user['SECOND_NAME'] : '';
				$this->entityData['RESPONSIBLE_LAST_NAME'] = isset($user['LAST_NAME']) ? $user['LAST_NAME'] : '';
				$this->entityData['RESPONSIBLE_PERSONAL_PHOTO'] = isset($user['PERSONAL_PHOTO']) ? $user['PERSONAL_PHOTO'] : '';
			}
		}
		//endregion
		...
		{
			$user = self::getUser($this->entityData['USER_ID']);
			if(is_array($user))
			{
				$this->entityData['USER_LOGIN'] = $user['LOGIN'];
				$this->entityData['USER_NAME'] = isset($user['NAME']) ? $user['NAME'] : '';
				$this->entityData['USER_SECOND_NAME'] = isset($user['SECOND_NAME']) ? $user['SECOND_NAME'] : '';
				$this->entityData['USER_LAST_NAME'] = isset($user['LAST_NAME']) ? $user['LAST_NAME'] : '';
				$this->entityData['USER_PERSONAL_PHOTO'] = isset($user['PERSONAL_PHOTO']) ? $user['PERSONAL_PHOTO'] : '';
			}
		}
		//endregion
		...
		//region User
		$userId = isset($this->entityData['USER_ID']) ? (int)$this->entityData['USER_ID'] : 0;
		if($userId > 0)
		{
			$this->entityData['USER_FORMATTED_NAME'] =
				\CUser::FormatName(
					$this->arResult['NAME_TEMPLATE'],
					array(
						'LOGIN' => $this->entityData['USER_LOGIN'],
						'NAME' => $this->entityData['USER_NAME'],
						'LAST_NAME' => $this->entityData['USER_LAST_NAME'],
						'SECOND_NAME' => $this->entityData['USER_SECOND_NAME']
					),
					true,
					false
				);

			$assignedByPhotoID = isset($this->entityData['USER_PERSONAL_PHOTO'])
				? (int)$this->entityData['USER_PERSONAL_PHOTO'] : 0;
			...
			if($assignedByPhotoID > 0)
			{
				...
				{
					$this->entityData['USER_PHOTO_URL'] = $fileInfo['src'];
				}
			}
		...
		//endregion
		//region Responsible
		$responsibleId = isset($this->entityData['RESPONSIBLE_ID']) ? (int)$this->entityData['RESPONSIBLE_ID'] : 0;
		if($responsibleId > 0)
		{
			$this->entityData['RESPONSIBLE_FORMATTED_NAME'] =
				\CUser::FormatName(
					$this->arResult['NAME_TEMPLATE'],
					array(
						'LOGIN' => $this->entityData['RESPONSIBLE_LOGIN'],
						'NAME' => $this->entityData['RESPONSIBLE_NAME'],
						'LAST_NAME' => $this->entityData['RESPONSIBLE_LAST_NAME'],
						'SECOND_NAME' => $this->entityData['RESPONSIBLE_SECOND_NAME']
					),
					true,
					false
				);

			$assignedByPhotoID = isset($this->entityData['RESPONSIBLE_PERSONAL_PHOTO'])
				? (int)$this->entityData['RESPONSIBLE_PERSONAL_PHOTO'] : 0;
			...
			if($assignedByPhotoID > 0)
			{
				...
				if(is_array($fileInfo) && isset($fileInfo['src']))
				{
					$this->entityData['RESPONSIBLE_PHOTO_URL'] = $fileInfo['src'];
				}
			}
	...
	protected function getShipmentEntityData()
	{
		...
		foreach ($shipments as $shipment)
		{
			...
			if($delivery)
			{
				$logoFileId = $delivery->getLogotip();
			...
			{
				$errors = $calcPrice->getErrorMessages();
			}
			$statusControlShipment = "";

			if ($this->mode !== ComponentMode::CREATION && $this->mode !== ComponentMode::COPING)
			{
			...
			$fields = array(
				...
				'PRICE_DELIVERY_CALCULATED' => $calcPrice->getPrice(),
				'FORMATTED_PRICE_DELIVERY_CALCULATED' => \CCrmCurrency::MoneyToString(
					$calcPrice->getPrice(),
					$this->entityData['CURRENCY'],
					'#'
				),
				'FORMATTED_PRICE_DELIVERY_CALCULATED_WITH_CURRENCY' => \CCrmCurrency::MoneyToString(
					$calcPrice->getPrice(),
					$this->entityData['CURRENCY'],
					''
				),
	...
	protected function getLastUserClients()
	{
		...
		$preparedData = \Bitrix\Crm\Controller\Entity::prepareSearchResults($clientData);
