...
if ($iDealId > 0)
{
	...
	if ($iVersion >= 2)
	{
		...
		{
			$fields .= '<span class="field-row">
				<span class="field-name">'.GetMessage('CRM_COLUMN_STAGE_ID').'</span>: <span class="field-value"><span class="fields enumeration">'.$arResult['STAGE_LIST'][$arDeal['STAGE_ID']].'</span></span>
			</span>';
		}
		if(count($arProductRows) > 0)
		{
			$fields .= '<span class="field-row">
				<span class="field-name">'.GetMessage('CRM_COLUMN_PRODUCTS').'</span>: <span class="field-value"><span class="fields enumeration">'.htmlspecialcharsbx(CCrmProductRow::RowsToString($arProductRows)).'</span></span>
			</span>';
		}
		if (!empty($arDeal['OPPORTUNITY']))
		{
			$fields .= '<span class="field-row">
				<span class="field-name">'.GetMessage('CRM_COLUMN_OPPORTUNITY').'</span>: <span class="field-value"><span class="fields enumeration"><nobr>'.number_format($arDeal['OPPORTUNITY'], 2, ',', ' ').' '.htmlspecialcharsbx(CCrmCurrency::GetCurrencyName($arDeal['CURRENCY_ID'])).'</nobr></span></span>
			</span>';
		}
		if (!empty($arDeal['PROBABILITY']))
		{
			$fields .= '<span class="field-row">
				<span class="field-name">'.GetMessage('CRM_COLUMN_PROBABILITY').'</span>: <span class="field-value"><span class="fields enumeration">'.intval($arDeal['PROBABILITY']).'%</span></span>
			</span>';
		}
		$fields .= '<span class="field-row">
			<span class="field-name">'.GetMessage('CRM_COLUMN_DATE_MODIFY').'</span>: <span class="field-value"><span class="fields enumeration">'.FormatDate('x', MakeTimeStamp($arDeal['DATE_MODIFY']), (time() + CTimeZone::GetOffset())).'</span></span>
		</span>';
		if (!empty($arDeal['COMPANY_TITLE']))
		{
			$fields .= '<span class="field-row">
				<span class="field-name">'.GetMessage('CRM_COLUMN_COMPANY_TITLE').'</span>: <span class="field-value"><a href="'.$arDeal['PATH_TO_COMPANY_SHOW'].'" target="_blank">'.htmlspecialcharsbx($arDeal['COMPANY_TITLE']).'</a></span>
			</span>';
		}
		if (!empty($arDeal['CONTACT_FORMATTED_NAME']))
		{
			$fields .= '<span class="field-row">
				<span class="field-name">'.GetMessage('CRM_COLUMN_CONTACT_FULL_NAME').'</span>: <span class="field-value"><a href="'.$arDeal['PATH_TO_CONTACT_SHOW'].'" target="_blank">'.htmlspecialcharsbx($arDeal['CONTACT_FORMATTED_NAME']).'</a></span>
			</span>';
		}
