			...
			else
			{
				...
				try
				{
					if(intval($this->request['TEMPLATE'])) // copy from template?
					{
						...
						$templateId = intval($this->request['TEMPLATE']);
						$sourceData = $this->cloneDataFromTemplate($templateId);

						$driver = \Bitrix\Disk\Driver::getInstance();
						$userFieldManager = $driver->getUserFieldManager();
						foreach ($userFieldManager->getAttachedObjectByEntity('TASKS_TASK_TEMPLATE', $templateId,'UF_TASK_WEBDAV_FILES') as $attachedObject)
						{
