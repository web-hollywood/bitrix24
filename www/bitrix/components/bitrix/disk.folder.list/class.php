...
class CDiskFolderListComponent extends DiskComponent implements \Bitrix\Main\Engine\Contract\Controllerable
{
		...
		if ($this->gridOptions->getViewMode() === FolderListOptions::VIEW_MODE_TILE)
		{
			...
			foreach ($this->arResult['GRID']['ROWS'] as $row)
			{
				...
				$info = [
					...
					'actions' => $row['actions'],
					'attributes' => $this->convertAttributesToArray($row['data']['VIEWER_ATTRS']),
				];
			...
			if($isFolder)
			{
				...
				$columnName = "
					<table class=\"bx-disk-object-name\"><tr>
							...
							<td><a class=\"bx-disk-folder-title js-disk-grid-folder\" id=\"disk_obj_{$objectId}\" href=\"{$exportData['OPEN_URL']}\" data-object-id=\"{$objectId}\" data-can-add=\"{$object->canAdd($securityContext)}\" {$dataAttributesForViewer}>{$nameSpecialChars}</a></td>
					</tr></table>
				";
			}
			else
	...
	protected function convertAttributesToArray($attributes)
	{
		...
		$array = [];
		$attributes = str_replace('bx-attach-file-id', 'data-bx-attach-file-id', $attributes);
		foreach (explode('data-bx-', $attributes) as $attr)
		{
			...
			$name = ucfirst(str_replace('-', '', strtolower($name)));
			$array["bx{$name}"] = rtrim($value, '" ');
		}
