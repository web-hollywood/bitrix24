...
class ImOpenLinesComponentStatisticsDetail extends CBitrixComponent
{
	...
	public static function getFormattedCrmColumn($row)
	{
		...
		$crmDealLink = self::getCrmDealLink($row["data"]);
		if ($crmDealLink)
		{
			$crmData[] = '<a href="'.$crmDealLink.'" target="_blank">'.self::getCrmName('DEAL').'</a>';
		}
	...
	private static function getCrmDealLink($row)
	{
		if (empty($row['CRM_DEAL_ID']))
		{
			return '';
		}

		return \Bitrix\ImOpenLines\Crm::getLink('DEAL', $row['CRM_DEAL_ID']);
	}
	...
	public function executeComponent()
	{
		...
		foreach($this->arResult["ELEMENTS_ROWS"] as $key => $row)
		{
			...
			if ($this->excelMode)
			{
				...
				$newRow["CRM_LINK"] = self::getCrmLink($row["data"]).' '.self::getCrmDealLink($row["data"]).' '.self::getCrmActivityLink($row["data"]);
			}
		...
		$this->arResult["HEADERS"] = array_merge($this->arResult["HEADERS"], Array(
			...
			array("id"=>"MESSAGE_COUNT", "name"=>GetMessage("OL_STATS_HEADER_MESSAGE_COUNT_NEW"), "default"=>true, "editable"=>false, "sort"=>"MESSAGE_COUNT"),
