	...
	protected function getHeaders()
	{
		...
		$result = array_merge(
			...
			array(
				...
				array('id' => 'USER', 'name' => Loc::getMessage('CRM_COLUMN_USER_ID'), 'sort' => 'USER', 'editable' => false),
...
//				array('id' => 'DELIVERY_ID', 'name' => Loc::getMessage('CRM_COLUMN_DELIVERY_ID'), 'sort' => 'DELIVERY_ID', 'editable' => false),
	...
	public function executeComponent()
	{
		...
		// POST & GET actions processing -->
		$this->requestProcessing($actionData, $filter);
		...
		foreach($this->arResult['ORDER'] as &$arOrder)
		{
			...
			$arOrder['RESPONSIBLE_BY'] = CUser::FormatName(
				$this->arParams['NAME_TEMPLATE'],
				array(
					'LOGIN' => isset($arOrder['RESPONSIBLE_BY_LOGIN']) ? htmlspecialcharsbx($arOrder['RESPONSIBLE_BY_LOGIN']) : '',
					'NAME' => isset($arOrder['RESPONSIBLE_BY_NAME']) ? htmlspecialcharsbx($arOrder['RESPONSIBLE_BY_NAME']) : '',
					'LAST_NAME' => isset($arOrder['RESPONSIBLE_BY_LAST_NAME']) ? htmlspecialcharsbx($arOrder['RESPONSIBLE_BY_LAST_NAME']) : '',
					'SECOND_NAME' => isset($arOrder['RESPONSIBLE_BY_SECOND_NAME']) ? htmlspecialcharsbx($arOrder['RESPONSIBLE_BY_SECOND_NAME']) : ''
				),
				true, false
			);
