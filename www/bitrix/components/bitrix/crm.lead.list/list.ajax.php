...
elseif ($action === 'PREPARE_BATCH_CONVERSION')
{
	...
	$progressData = array('CONFIG' => $config->externalize(), 'ENTITY_IDS' => $IDs, 'CURRENT_ENTITY_INDEX' => 0, 'CURRENT_ENTITY_ID' => 0, 'PROCESSED_ENTITIES' => 0);
...
elseif ($action === 'PROCESS_BATCH_CONVERSION')
{
	...
	$entityIDs = isset($progressData['ENTITY_IDS']) ? $progressData['ENTITY_IDS'] : array();
	$entityQty = count($entityIDs);
	...
	if($currentEntityID <= 0 && !empty($entityIDs))
	{
	...
	if($errorText !== '')
	{
		...
		$resultData['ERROR_EXTRAS'] = array('INFO' => $entityInfo);
	}
